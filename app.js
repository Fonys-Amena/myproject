...
var driver = neo4j.driver("bolt://localhost", neo4j.auth.basic("neo4j", "admin123"));
...
app.get('/', function (req, res) {
    ...
    //res.render('index');    //res.send('It Works!');
});
...
app.post('/person/born/add', function (req, res) {
    ...
    session
        .run("MATCH(a:Person {name:$nameParam}),(b:Location {city:$cityParam,state:$stateParam}) MERGE(a)-[r:BORN_IN]->(b) RETURN a,b", {nameParam: name, cityParam: city, stateParam:state, yearParam:year })
        .then(function (result) {
            if (id && id != null) {
                res.redirect('/person/'+id);
            } else {
                ...
                .then(function (result2) {
                    ...
                    session
                        ...
                        .then(function (result3) {
                            ...
                            //session.close();  // when close session applied, the page can't go back from person.ejs
                        })
                        .catch(function (error) {
                            console.log(error);
                        });
